...
def show_prob_drones(p, image):
    ...
    ft = 15
    label = (
    "boat", "building", "car", "drone", "group", "horseride", "paraglider", "person", "riding", "truck", "wakeboard",
    "whale")
...
def get_IOU (pred_frame, gt_frame):
    (x1, y1, w1, h1) = pred_frame
    (x2, y2, w2, h2) = gt_frame
...
def get_mean_IOU (pred_frames, gt_frames):
    IOU = 0
    for i in range(len(pred_frames)):
        IOU += get_IOU(pred_frames[i], gt_frames[i])
    return IOU / len(pred_frames)
